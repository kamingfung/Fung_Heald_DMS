...
! !USES:
  ...
  use modal_aero_data, only:  ntot_aspectype=>nspec_max, ntot_amode, nsoa, npoa, nbc
! use ref_pres,        only:  top_lev => clim_modal_aero_top_lev  ! this is for gg02a
  ...
  ! to save aerosol pH (dsj) fkm: for MOSAIC pH
...
! controls updating of dgncur_awet and wetdens_host
  ...
  logical, parameter :: mwaer_from_advmass = .false.
! true (or false) - mw_aer_host set from adv_mass ( or specmw_amode )
  ...
  integer :: ntot_amode_extd

  integer, parameter :: max_mode_fresh = 1

  integer, public, protected :: max_mode
  ...
  integer :: igas_soa2, igas_soa3, igas_soa4, igas_soa5
  ...
  integer :: iaer_bc, iaer_dst, iaer_ncl, iaer_nh4, iaer_pom, iaer_soa, iaer_so4, &
             iaer_soa2, iaer_soa3, iaer_soa4, iaer_soa5, &
  ...
  integer, allocatable :: lmap_gas(:)
  integer, allocatable :: lmap_aer(:,:), lmapbb_aer(:,:), &
             lmap_aercw(:,:)
  integer, allocatable :: lmap_num(:), lmap_numcw(:)
  ...
  integer, allocatable :: mode_aging_optaa(:)
  ...
  real(r8), allocatable :: accom_coef_gas(:)
  real(r8), allocatable :: alnsg_aer(:)
  real(r8), allocatable :: dgnum_aer(:), dgnumhi_aer(:), dgnumlo_aer(:)
  real(r8), allocatable :: dens_aer(:)
  real(r8) :: dens_so4a_host
  real(r8), allocatable :: fac_m2v_aer(:)        ! converts (mol-aero/mol-air) to (m3-aero/mol-air)
  real(r8), allocatable :: fac_eqvso4hyg_aer(:)  ! converts a species volume to a volume of so4
                                          !    (or nh4hso4) having same hygroscopicity
  real(r8), allocatable :: fac_m2v_eqvhyg_aer(:) ! = fac_m2v_aer * fac_eqvso4hyg_aer

  real(r8), allocatable :: fcvt_gas(:), fcvt_aer(:)
  real(r8) :: fcvt_num, fcvt_wtr
  real(r8), allocatable :: fcvt_dgnum_dvolmean(:)
  real(r8), allocatable :: hygro_aer(:)
  real(r8), allocatable :: mw_gas(:), mw_aer(:)
  real(r8), allocatable :: mwhost_gas(:), mwhost_aer(:)
  real(r8) :: mwhost_num
  real(r8) :: mw_nh4a_host, mw_so4a_host
  real(r8), allocatable :: mwuse_soa(:), mwuse_poa(:)
  real(r8), allocatable :: sigmag_aer(:)
  real(r8), allocatable :: vol_molar_gas(:)
...
!    soa_equivso4_factor(:)
  ...
  character(len=16), allocatable :: name_gas(:), name_aerpfx(:), &
     name_aer(:,:), name_aercw(:,:), &
     name_num(:), name_numcw(:)
...
#if ( defined( MOSAIC_SPECIES ) )
     real(r8), public :: hygro_bc=0.0_r8, hygro_pom=0.0_r8, hygro_soa=0.0_r8, hygro_dst=0.0_r8
#endif
...
! dsj for saving pH fields to buffer; fkm for MOSAIC pH
!                        wetdens_host,                            &
                        pbuf,               wetdens_host,        &
                        qaerwat                                  )
                        ! wetdens_host )
...
! for saving pH to buffer (dsj) fkm for MOSAIC pH
...
! !PARAMETERS:
   ...
   type(physics_buffer_desc), pointer :: pbuf(:)    ! fkm for MOSAIC pH
...
! to save aerosol pH to history file (dsj)  fkm for MOSAIC pH
      real(r8) :: pH_fields(pcols,pver,nbin_a_max)
      real(r8) :: Hconc_fields(pcols,pver,nbin_a_max)
      real(r8) :: count_qaerwat(pcols,pver,nbin_a_max)
      real(r8) :: count_no_qaerwat(pcols,pver,nbin_a_max)
      real(r8) :: count_no_Hconc(pcols,pver,nbin_a_max)
      real(r8) :: Hconc_sav(nbin_a_max)

      real(r8) :: HSO4_fields(pcols,pver,nbin_a_max)
      real(r8) :: SO4_fields(pcols,pver,nbin_a_max)
      real(r8) :: NO3_fields(pcols,pver,nbin_a_max)
      real(r8) :: HSO4conc_sav(nbin_a_max)
      real(r8) :: SO4conc_sav(nbin_a_max)
      real(r8) :: NO3conc_sav(nbin_a_max)
      
! to save aerosol pH and properties to buffer (dsj)  fkm for MOSAIC pH
      integer  :: pH_index, HSO4_index, SO4_index, NO3_index
      real(r8), pointer, dimension(:,:,:) :: pH_buf, HSO4_buf
      real(r8), pointer, dimension(:,:,:) :: SO4_buf, NO3_buf
...
!        qsub_tendaa, qqcwsub_tendaa              )
 ...
 ! to save aerosol pH (dsj) fkm for MOSAIC pH
 !         misc_vars_aa                             )
          misc_vars_aa, Hconc_sav, HSO4conc_sav,   &
          SO4conc_sav,  NO3conc_sav )
...
! diagnostics after forming sub-areas
      ...
      do l2 = 1, 4
         ...
         write(lun13n,'(2a,1p,4e12.4)') tmpch6a, ' host  1-4', &
            q_pregaschem(i,k,l)*tmpa, q_precldchem(i,k,l)*tmpa, 0.0_r8, q(i,k,l)*tmpa
...
! increment column tendencies
      ...
      ! to save aerosol pH (dsj) fkm for MOSAIC pH
      do l = 1, nbin_a_max
         !pH_fields(i,k,l) = -log10( Hconc_sav(l) )
         pH_fields(i,k,l) = MIN(14.0_r8, -log10(Hconc_sav(l)) )
         Hconc_fields(i,k,l) = Hconc_sav(l)
         HSO4_fields(i,k,l) = HSO4conc_sav(l)
         SO4_fields(i,k,l) = SO4conc_sav(l)
         NO3_fields(i,k,l) = NO3conc_sav(l)
         
         if ( qaerwatgcm4(l) > 0.0_r8 ) then
            count_qaerwat(i,k,l) = count_qaerwat(i,k,l) + 1
         else
            count_no_qaerwat(i,k,l) = count_no_qaerwat(i,k,l) + 1
         endif

         if ( Hconc_sav(l) == 0.0_r8 ) then
            count_no_Hconc(i,k,l) = count_no_Hconc(i,k,l) + 1            
         endif
      end do
...
!    but is not important for regular cam simulations
      ...
      if ( mosaic ) then
         ...
         ! to save aerosol pH (dsj)
         call outfld( 'aerosol_ph_bin1', pH_fields(1:ncol,:,1), ncol, lchnk )
         call outfld( 'aerosol_ph_bin2', pH_fields(1:ncol,:,2), ncol, lchnk )
         call outfld( 'aerosol_ph_bin3', pH_fields(1:ncol,:,3), ncol, lchnk )
         call outfld( 'aerosol_ph_bin4', pH_fields(1:ncol,:,4), ncol, lchnk )

         call outfld( 'hconc_bin1', Hconc_fields(1:ncol,:,1), ncol, lchnk )
         call outfld( 'hconc_bin2', Hconc_fields(1:ncol,:,2), ncol, lchnk )
         call outfld( 'hconc_bin3', Hconc_fields(1:ncol,:,3), ncol, lchnk )
         call outfld( 'hconc_bin4', Hconc_fields(1:ncol,:,4), ncol, lchnk )

         call outfld( 'n_aerwat_bin1', count_qaerwat(1:ncol,:,1), ncol, lchnk )
         call outfld( 'n_aerwat_bin2', count_qaerwat(1:ncol,:,2), ncol, lchnk )
         call outfld( 'n_aerwat_bin3', count_qaerwat(1:ncol,:,3), ncol, lchnk )
         call outfld( 'n_aerwat_bin4', count_qaerwat(1:ncol,:,4), ncol, lchnk )

         call outfld( 'n_noaerwat_bin1', count_no_qaerwat(1:ncol,:,1), ncol, lchnk )
         call outfld( 'n_noaerwat_bin2', count_no_qaerwat(1:ncol,:,2), ncol, lchnk )
         call outfld( 'n_noaerwat_bin3', count_no_qaerwat(1:ncol,:,3), ncol, lchnk )
         call outfld( 'n_noaerwat_bin4', count_no_qaerwat(1:ncol,:,4), ncol, lchnk )

         call outfld( 'n_aerosol_ph_bin1', count_no_Hconc(1:ncol,:,1), ncol, lchnk )
         call outfld( 'n_aerosol_ph_bin2', count_no_Hconc(1:ncol,:,2), ncol, lchnk )
         call outfld( 'n_aerosol_ph_bin3', count_no_Hconc(1:ncol,:,3), ncol, lchnk )
         call outfld( 'n_aerosol_ph_bin4', count_no_Hconc(1:ncol,:,4), ncol, lchnk )
         !!! ==== [END: fkm for MOSAIC pH ] =====
         
        !!! ==== [BEG: fkm for MOSAIC aLWC ] =====
        call outfld( 'qaerwat_bin1', qaerwat(1:ncol,:,1), ncol, lchnk )
        call outfld( 'qaerwat_bin2', qaerwat(1:ncol,:,2), ncol, lchnk )
        call outfld( 'qaerwat_bin3', qaerwat(1:ncol,:,3), ncol, lchnk )
        call outfld( 'qaerwat_bin4', qaerwat(1:ncol,:,4), ncol, lchnk )

        !!! ==== [END: fkm for MOSAIC aLWC ] =====

         ! save aerosol pH to buffer for the use of user reaction calculations in the next time step (dsj)
         ! For now, use different method
         
         !!! ==== [BEG: fkm for MOSAIC pH ] =====
         pH_index = pbuf_get_index('MOSAIC_pH')
         call pbuf_get_field(pbuf, pH_index, pH_buf)
         do n = 1, 4
           pH_buf(:,:,n) = pH_fields(:,:,n)
         end do
        !!! ==== [END: fkm for MOSAIC pH ] =====
        
         !HSO4_index = pbuf_get_index('MOSAIC_HSO4')
         !call pbuf_get_field(pbuf, HSO4_index, HSO4_buf)
         !do n = 1, 4
         !   HSO4_buf(:,:,n) = HSO4_fields(:,:,n)
         !end do
         !SO4_index = pbuf_get_index('MOSAIC_SO4')
         !call pbuf_get_field(pbuf, SO4_index, SO4_buf)
         !do n = 1, 4
         !   SO4_buf(:,:,n) = SO4_fields(:,:,n)
         !end do
         !NO3_index = pbuf_get_index('MOSAIC_NO3')
         !call pbuf_get_field(pbuf, NO3_index, NO3_buf)
         !do n = 1, 4
         !   NO3_buf(:,:,n) = NO3_fields(:,:,n)
         !end do

         do n = 1, 4
            MOSAIC_pH(:,:,n) = pH_fields(:,:,n)
         end do
         do n = 1, 4
            MOSAIC_HSO4(:,:,n) = HSO4_fields(:,:,n)
         end do
         do n = 1, 4
            MOSAIC_SO4(:,:,n) = SO4_fields(:,:,n)
         end do
         do n = 1, 4
            MOSAIC_NO3(:,:,n) = NO3_fields(:,:,n)
         end do
         !!! ==== [END: fkm for MOSAIC pH ] =====
      end if
      ...
      if (mosaic_gaex_prodloss3d > 0) then
         do l = 1, pcnst-loffset   ! skip any species for which l > pcnst-loffset
            if (mosaic_gaex_prodloss3d_ga(l+loffset) > 0) then
               tmpa_ik(1:ncol,1:pver) = max( q_condtend_ikl(1:ncol,1:pver,l), 0.0_r8 ) * (adv_mass(l)/mwdry)
               fieldname = trim(cnst_name(l+loffset)) // '_gaex_prod3d'
               call outfld( fieldname, tmpa_ik, ncol, lchnk )
               tmpa_ik(1:ncol,1:pver) = min( q_condtend_ikl(1:ncol,1:pver,l), 0.0_r8 ) * (adv_mass(l)/mwdry)
...
!    renaming, coagulation, and nucleation
      ...
      real(r8), intent(inout) :: HSO4conc_sav(nbin_a_max)
      real(r8), intent(inout) :: SO4conc_sav(nbin_a_max)
      real(r8), intent(inout) :: NO3conc_sav(nbin_a_max)
...
!         misc_vars_aa_sub(jsub)                      )
          misc_vars_aa_sub(jsub), Hconc_sav,          &
          HSO4conc_sav, SO4conc_sav,  NO3conc_sav )
...
! to save aerosol pH (dsj) fkm for MOSAIC pH
!         misc_vars_aa_sub                            )
        misc_vars_aa_sub, Hconc_sav, HSO4conc_sav,  &
        SO4conc_sav,  NO3conc_sav )
!
! calculates changes to gas and aerosol sub-area TMRs (tracer mixing ratios)
...
!              uptkrate_h2so4,    misc_vars_aa_sub                        )
              uptkrate_h2so4,    misc_vars_aa_sub, Hconc_sav,            &
              HSO4conc_sav,  SO4conc_sav,  NO3conc_sav )
      else
#endif
...
!---------------------------------------------------------------------
      ...
      subroutine mosaic_gasaerexch_1subarea_intr(  nstep,                &!Intent(ins)
              ...
              qgas_cur,          qnum_cur,         qwtr_cur,             &
              qgas_avg,          qgas_netprod_otrproc,                   &
! to save aerosol pH  fkm for MOSAIC pH
!              uptkrate_h2so4,    misc_vars_aa_sub                        )
              uptkrate_h2so4,    misc_vars_aa_sub, Hconc_sav,            &
              HSO4conc_sav, SO4conc_sav,  NO3conc_sav )
        !------------------------------------------------------------------------------!
        ...
        use module_data_mosaic_aero,   only: naer_mosaic => naer, &
             ...
             ilim1_a, iaro1_a, iapi1_a, iapi2_a, &  !zlu soa
             ilim2_g, ih2so4_g, inh3_g, ihno3_g, ihcl_g, &
             ilim1_g, iaro1_g, iapi1_g, iapi2_g, & !zlu soa
             jhyst_up, jtotal, &
             ! move nbin_a_max to module use variable  ! fkm for MOSAIC pH
             !nbin_a, nbin_a_max, ngas_volatile, nmax_astem, nmax_mesa, nsalt, &
             nbin_a, ngas_volatile, nmax_astem, nmax_mesa, nsalt, &
             mosaic_vars_aa_type
#ifdef SPMD
...
!       it_mosaic   = nstep
        ...
        do imode = 1, n_mode
           ...
           !
           aer(ilim1_a, jtotal, imode)  = qaer_cur(iaer_soa2, imode) * nano_mult_cair
           aer(iaro1_a, jtotal, imode)  = qaer_cur(iaer_soa3, imode) * nano_mult_cair
           aer(iapi1_a, jtotal, imode)  = qaer_cur(iaer_soa4, imode) * nano_mult_cair
           aer(iapi2_a, jtotal, imode)  = qaer_cur(iaer_soa5, imode) * nano_mult_cair
        ...
        !
        gas(ilim1_g)  = qgas_cur(igas_soa2)  * nano_mult_cair
        gas(iaro1_g)  = qgas_cur(igas_soa3)  * nano_mult_cair
        gas(iapi1_g)  = qgas_cur(igas_soa4)  * nano_mult_cair
        gas(iapi2_g)  = qgas_cur(igas_soa5)  * nano_mult_cair
...
!       hostgridinfo(4:6) = bigint
        ...
        kappa_nonelectro(ilim1_a) = hygro_soa
        kappa_nonelectro(iaro1_a) = hygro_soa
        kappa_nonelectro(iapi1_a) = hygro_soa
        kappa_nonelectro(iapi2_a) = hygro_soa
...
!             uptkrate_h2so4,          gam_ratio,    jaerosolstate_bgn                   )
            uptkrate_h2so4,          gam_ratio,    jaerosolstate_bgn, Hconc_sav,       &
            HSO4conc_sav,  SO4conc_sav,  NO3conc_sav )
...
!       uptkrate_h2so4,         gam_ratio,    jaerosolstate_bgn                   )
        ...
        do imode = 1, n_mode
           ...
           qaer_cur(iaer_soa2,imode) = aer(ilim1_a, jtotal , imode) * nano_mult_cair_inv
           qaer_cur(iaer_soa3,imode) = aer(iaro1_a, jtotal , imode) * nano_mult_cair_inv
           qaer_cur(iaer_soa4,imode) = aer(iapi1_a, jtotal , imode) * nano_mult_cair_inv
           qaer_cur(iaer_soa5,imode) = aer(iapi2_a, jtotal , imode) * nano_mult_cair_inv
        ...
        qgas_cur(igas_soa2)   = gas(ilim1_g) * nano_mult_cair_inv
        qgas_cur(igas_soa3)   = gas(iaro1_g) * nano_mult_cair_inv
        qgas_cur(igas_soa4)   = gas(iapi1_g) * nano_mult_cair_inv
        qgas_cur(igas_soa5)   = gas(iapi2_g) * nano_mult_cair_inv
        ...
        qgas_avg(igas_soa2)   = gas_avg(ilim1_g)  * nano_mult_cair_inv
        qgas_avg(igas_soa3)   = gas_avg(iaro1_g)  * nano_mult_cair_inv
        qgas_avg(igas_soa4)   = gas_avg(iapi1_g)  * nano_mult_cair_inv
        qgas_avg(igas_soa5)   = gas_avg(iapi2_g)  * nano_mult_cair_inv        
...
! fix a bug here by Dick Apr15/2016, zlu 
...
!     real(r8), parameter :: dtsub_fixed = 10.0    ! fixed sub-step for time integration (s)
      ...
      real(r8) :: a_soa(nsoa,max_mode)     ! soa aerosol mixrat (mol/mol at actual mw)
      real(r8) :: a_soa_tmp(nsoa,max_mode) ! temporary soa aerosol mixrat (mol/mol)
      real(r8) :: beta(nsoa,max_mode)      ! dtcur*xferrate
      real(r8) :: delh_vap_soa(nsoa)       ! delh_vap_soa = heat of vaporization for gas soa (J/mol)
      real(r8) :: del_g_soa_tmp(nsoa)
      real(r8) :: dtcur                            ! current time step (s)
      real(r8) :: dtfull                           ! full time step (s)
      real(r8) :: dtmax                            ! = (dtfull-tcur)
      real(r8) :: dtsum_qgas_avg
      real(r8) :: g0_soa(nsoa)             ! ambient soa gas equilib mixrat (mol/mol at actual mw)
      real(r8) :: g_soa(nsoa)              ! soa gas mixrat (mol/mol at actual mw)
      real(r8) :: g_star(nsoa,max_mode)    ! soa gas mixrat that is in equilib
                                                   ! with each aerosol mode (mol/mol)
      real(r8) :: mw_poa(npoa)             ! actual molec wght of poa
      real(r8) :: mw_soa(nsoa)             ! actual molec wght of soa
      real(r8) :: opoa_frac(npoa,max_mode) ! fraction of poa that is opoa
      real(r8) :: phi(nsoa,max_mode)       ! "relative driving force"
      real(r8) :: p0_soa(nsoa)             ! soa gas equilib vapor presssure (atm)
      real(r8) :: p0_soa_298(nsoa)         ! p0_soa_298 = soa gas equilib vapor presssure (atm) at 298 k
      real(r8) :: sat(nsoa,max_mode)       ! sat(m,ll) = g0_soa(ll)/a_ooa_sum_tmp(m) = g_star(m,ll)/a_soa(m,ll)
                                                   !    used by the numerical integration scheme -- it is not a saturation rato!
      real(r8) :: tcur                             ! current integration time (from 0 s)

      real(r8) :: tmpa, tmpb, tmpc

      real(r8) :: tot_soa(nsoa)            ! g_soa + sum( a_soa(:) )
...
! also compute dry-volume change during the continuous growth process
      ...
      do n = 1, ntot_amode
         ...
         npair = npair + 1
         mfrm = n
         factoraa(mfrm) = (pi/6._r8)*exp(4.5_r8*(alnsg_aer(mfrm)**2))
         factoraa(mtoo) = (pi/6._r8)*exp(4.5_r8*(alnsg_aer(mtoo)**2))
         factoryy(mfrm) = sqrt( 0.5_r8 )/alnsg_aer(mfrm)
...
!        v2nhirlx(mfrm) = voltonumbhi_amode(mfrm)/frelax
         ...
         tmp_alnsg2(mfrm) = 3.0_r8 * (alnsg_aer(mfrm)**2)
         dp_cut(mfrm) = sqrt(   &
            dgnum_aer(mfrm)*exp(1.5_r8*(alnsg_aer(mfrm)**2)) *   &
            dgnum_aer(mtoo)*exp(1.5_r8*(alnsg_aer(mtoo)**2)) )
...
!
      fac_volsfc = exp( 2.5_r8*(alnsg_aer(nfrm)**2) )
      xferfrac_max = 1.0_r8 - 10.0_r8*epsilon(1.0_r8)   ! 1-eps
...
!--------------------------------------------------------------------------------
!--------------------------------------------------------------------------------
      subroutine modal_aero_amicphys_init( loffset )
...
use ppgrid, only       :  pcols, pver ! dsj; fkm for MOSAIC pH
use modal_aero_data, only : MOSAIC_pH, MOSAIC_HSO4, &
                            MOSAIC_SO4, MOSAIC_NO3  ! dsj; fkm for MOSAIC pH
...
use constituents, only : cnst_species_class
use modal_aero_data, only : spechygro, specdens_amode, specmw_amode
...
! local
   ...
   max_gas = nsoa + 1
   max_aer = nsoa + npoa + nbc + 3
#elif ( ( defined MODAL_AERO_4MODE ) && ( defined MOSAIC_SPECIES ) )
   max_gas = nsoa + 4
   max_aer = nsoa + npoa + nbc + 8    !zlu NH4
#elif ( defined MODAL_AERO_4MODE )
   max_gas = nsoa + 1
   max_aer = nsoa + npoa + nbc + 3
#elif ( ( defined MODAL_AERO_7MODE ) && ( defined MOSAIC_SPECIES ) )
   max_gas = nsoa + 4
   max_aer = nsoa + npoa + nbc + 8
#elif ( defined MODAL_AERO_7MODE )
   max_gas = nsoa + 2
   max_aer = nsoa + npoa + nbc + 4
#elif ( defined MODAL_AERO_8MODE )
   max_gas = nsoa + 2
   max_aer = nsoa + npoa + nbc + 4
#elif ( defined MODAL_AERO_9MODE )
   max_gas = nsoa + 2
   max_aer = nsoa + npoa + nbc + 4 + 5
#endif

#if ( defined MODAL_AERO_8MODE ) || ( defined MODAL_AERO_4MODE )
   ntot_amode_extd = ntot_amode
#else
   ntot_amode_extd = ntot_amode + 1
#endif

   max_mode = ntot_amode_extd + max_mode_fresh

   allocate( lmap_gas(max_gas) )
   allocate( lmap_aer(max_aer,max_mode), lmapbb_aer(max_aer,max_mode), &
             lmap_aercw(max_aer,max_mode) )
   allocate( lmap_num(max_mode), lmap_numcw(max_mode) )
   allocate(mode_aging_optaa(max_mode))
   allocate(accom_coef_gas(max_gas))
   allocate( alnsg_aer(max_mode))
   allocate( dgnum_aer(max_mode), dgnumhi_aer(max_mode), dgnumlo_aer(max_mode))
   allocate( dens_aer(max_aer))
   allocate(fac_m2v_aer(max_aer))
   allocate(fac_eqvso4hyg_aer(max_aer))
   allocate( fac_m2v_eqvhyg_aer(max_aer) )
   allocate(fcvt_gas(max_gas), fcvt_aer(max_aer) )
   allocate(fcvt_dgnum_dvolmean(max_mode))
   allocate(hygro_aer(max_aer))
   allocate(mw_gas(max_gas), mw_aer(max_aer))
   allocate(mwhost_gas(max_gas), mwhost_aer(max_aer))

   allocate(mwuse_soa(nsoa), mwuse_poa(npoa))
   allocate(sigmag_aer(max_mode))
   allocate(vol_molar_gas(max_gas))

   allocate( name_gas(max_gas), name_aerpfx(max_aer), &
     name_aer(max_aer,max_mode), name_aercw(max_aer,max_mode), &
     name_num(max_mode), name_numcw(max_mode) )

     allocate( MOSAIC_pH(pcols,pver,ntot_amode), &
               MOSAIC_HSO4(pcols,pver,ntot_amode), &
               MOSAIC_SO4(pcols,pver,ntot_amode), &
               MOSAIC_NO3(pcols,pver,ntot_amode) )   ! fkm for MOSAIC pH
...
!zlu soa
      ...
      jsoa = 0
      do n = 1,pcnst
         name = trim(cnst_name(n))
         if (index(name,'SOAG')>0) then
            jsoa = jsoa+1
            name_gas(jsoa) = trim(name)
            write(numstr,fmt=fmtstr) jsoa
            name_aerpfx(jsoa) = 'soa'//trim(numstr)
         endif
      enddo
      ...
      igas_soa = 1
      iaer_soa = 1
!zlu soa add more index for all SOAG ?
     if (nsoa == 5) then
         igas_soa2 = 2
         iaer_soa2 = 2
         igas_soa3 = 3
...
! set aerosol mass and number mapping
      ...
      do n = 1, ntot_amode
         ...
         do l1 = 1, nspec_amode(n)
            ...
            if (iaer <= 0) then
               msg = 'modal_aero_amicphys_init ERROR - lmap_aer for ' // tmpnamea//' '//tmpnameb
               call endrun( msg )
            end if
            if (lac == 1) then
               ...
               dens_aer(iaer) = specdens_amode(l1,n)
               hygro_aer(iaer) = spechygro(l1,n)
               if ( mwaer_from_advmass ) then
                  mwhost_aer(iaer) = adv_mass(lmz)
               else
                  mwhost_aer(iaer) = specmw_amode(l1,n)
               end if
...
! diagnostics
      if ( masterproc ) then
         ...
         do n = 1, ntot_amode
            ...
            do iaer = 1, naer
               if (lmap_aer(iaer,n) > 0) then
                  ...
                  tmpa = mwhost_aer(iaer) ; tmpb = adv_mass(lmap_aer(iaer,n))
                  if (abs(tmpa-tmpb) > 0.15_r8) then
                     write(msg,'(2a,2f10.3)') 'specmw - adv_mass error ', &
                        trim(name_aer(iaer,n)), tmpa, tmpb
                     write(iulog,'(a)') msg
                     nerr = nerr + 1
                  end if
               end if
         ...
         if (nerr > 0) then
            write(iulog,'(a,i5/)') '*** amicphys - specmw adv_mass mismatch nerr =', nerr
!           call endrun( '*** amicphys - specmw adv_mass mismatch error' )
         end if
         ...
         do lmz = 1, gas_pcnst
            l = lmz + loffset
            j = -99
            if (l <= pcnst) j = cnst_species_class(l)
            write(iulog,'(4i5,2x,a)') &
               lmz+loffset, lmz, lmapcc_all(lmz), j, solsym(lmz)
         end do

#if ( defined MOSAIC_SPECIES )
         write(iulog,'(/a,1p,4e14.6/)') 'hygro for mosaic:  bc, pom, soa, dst =', &
            hygro_bc, hygro_pom, hygro_soa, hygro_dst
#endif
...
!--------------------------------------------------------------------------------
      ...
      use module_data_mosaic_aero, only: &
         ...
         ilim1_g, iaro1_g, iapi1_g, iapi2_g, &
         iso4_a, ino3_a, icl_a, inh4_a, ina_a, ioin_a, ica_a, ico3_a, &
         ilim2_a, ioc_a, ibc_a, &
         ilim1_a, iaro1_a, iapi1_a, iapi2_a, &
      ...
      do igas = 1, ngas
         if      (igas == igas_h2so4) then
            ...
            i = ilim2_g
         else if (igas == igas_soa2) then   !zlu soa
            i = ilim1_g
         else if (igas == igas_soa3) then
            i = iaro1_g
         else if (igas == igas_soa4) then
            i = iapi1_g
         else if (igas == igas_soa5) then
            i = iapi2_g         
         else
            i = 0
         end if
      ...
      do iaer = 1, naer
         if      (iaer == iaer_so4) then
            ...
            i = ilim2_a
         else if (iaer == iaer_soa2) then
            i = ilim1_a
         else if (iaer == iaer_soa3) then
            i = iaro1_a
         else if (iaer == iaer_soa4) then
            i = iapi1_a
         else if (iaer == iaer_soa5) then
            i = iapi2_a      
         ...
         if (i > 0) then
            ...
            if (abs(mw_aer(iaer)-tmpa) > 0.15_r8) then
               if ( iaer==iaer_dst .or. iaer==iaer_pom .or. iaer==iaer_bc ) then
                  tmpcha = '-----'
               else
                  tmpcha = '*****'
                  nerr = nerr + 1
...
!  define history fields for nucleation
      ...
      if ( mosaic ) then
         ...
         call addfld('max_kelvin_iter',  (/ 'lev' /), 'A', 'no units','For tracking MOSAIC convergence failure' )
         ...
         do m = 1, 5
            fieldname = ' '
            write( fieldname(1:16), '(a,i1,a,i1)') 'astem_negval_', m, '_', n
            call addfld( fieldname, (/ 'lev' /), 'A', 'no units','For tracking MOSAIC convergence failure' )
            call add_default( fieldname, 1, ' ' )
         end do
         end do
      end if
