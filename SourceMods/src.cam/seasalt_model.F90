...
#if ( defined( MOSAIC_SPECIES ) )
    integer :: icl,iso4
    real(r8), parameter :: fracemit_so4 = 0._r8 ! 0.077_r8 fkm: zeroing SS-sulfate                           
       ...
       do i=1, nsections
          if (Dg(i).ge.sst_sz_range_lo(ibin) .and. Dg(i).lt.sst_sz_range_hi(ibin)) then
             ...
             ! cflx(:ncol,iso4)= cflx(:ncol,iso4)+fi(:ncol,i)*ocnfrc(:ncol)*emis_scale &   !++ ag: scale sea-salt
                             ! * 4._r8/3._r8*pi*rdry(i)**3*dns_aer_sst* fracemit_so4    ! commented by fkm: 7.7% of seasalt emission is considered as so4_a{n}
             cflx(:ncol,iso4)= 0._r8    ! added by fkm for DMS only
#else
             cflx(:ncol,mm) = cflx(:ncol,mm)+fi(:ncol,i)*ocnfrc(:ncol)*emis_scale  &   !++ ag: scale sea-salt
