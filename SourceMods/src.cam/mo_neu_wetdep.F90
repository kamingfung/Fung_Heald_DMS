...
!
  ...
  integer                     :: msia_ndx, msa_ndx    ! fkm: for deposition
  logical                     :: debug   = .false.
  integer                     :: hno3_ndx = 0
  integer                     :: h2o2_ndx = 0
!
...
!
    ...
    ! ===== [BEG: added by fkm for deposition] =====
    if ( trim(test_name) == 'MSIA' ) then
      msia_ndx = m
    end if
    
    if ( trim(test_name) == 'MSA' ) then
      msa_ndx = m
    end if
    ! ===== [END: added by fkm for deposition] =====
!
  end do
...
!
  if ( debug ) then
    ...
    print '(a,i4)','so2_ndx',so2_ndx
    print '(a,i4)','msia_ndx',msia_ndx    ! fkm for deposition
    print '(a,i4)','msa_ndx',msa_ndx      ! fkm for deposition
  end if
!
...
!
      if( dheff(id+5) /= 0._r8 ) then
        ...
        if( nh3_ndx > 0 .or. co2_ndx > 0 .or. so2_ndx > 0 .or. msia_ndx > 0 .or. msa_ndx > 0) then    ! fkm: added for deposition
          e298 = dheff(id+3)
          dhr  = dheff(id+4)
          dk1s(:) = e298*exp( dhr*wrk(:) )
          e298 = dheff(id+5)
          ...
          if( m == co2_ndx .or. m == so2_ndx .or. msia_ndx > 0 .or. msa_ndx > 0) then   ! fkm: added for deposition
