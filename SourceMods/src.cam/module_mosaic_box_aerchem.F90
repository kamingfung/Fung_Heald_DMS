...
! add indices to save aerosol concentrations (dsj) fkm: for MOSAIC pH
         jc_h, ja_hso4, ja_so4, ja_no3
...
!zlu soa
    real(r8) ::  tot_lim1_in, tot_aro1_in, tot_api1_in, tot_api2_in
...
!    save the gas,aer,num_a,... arrays then restore them for each repeat cycle
    do irepeat_mosaic = 1, 1
       ...
       call overall_massbal_in( aer, gas, gas_netprod_otrproc, dtchem,                  & !intent-ins
            ...
            tot_ca_in, tot_lim2_in, tot_lim1_in, tot_aro1_in, tot_api1_in, tot_api2_in )
       ...
       call overall_massbal_out( iprint_input, 0, mosaic_vars_aa%isteps_ASTEM, aer, gas,   &
          tot_so4_in, tot_no3_in, tot_cl_in, tot_nh4_in, tot_na_in, tot_ca_in, tot_lim2_in,&
               tot_lim1_in, tot_aro1_in, tot_api1_in, tot_api2_in, mosaic_vars_aa )

    enddo ! irepeat
    
    ! to save aerosol pH and properties (dsj) fkm: for MOSAIC pH
    Hconc_sav(:) = mc(jc_h,:)
    HSO4conc_sav(:) = ma(ja_hso4,:)
    SO4conc_sav(:) = ma(ja_so4,:)
    NO3conc_sav(:) = ma(ja_no3,:)
...
!zlu soa
    ...
    complex(r8), dimension(nbin_a_max) :: ri_shell_a,ri_avg_a,ri_core_a
    ...
    call overall_massbal_in( aer, gas, gas_netprod_otrproc, dtchem,                  & !intent-ins
         total_species, tot_so4_in, tot_no3_in, tot_cl_in, tot_nh4_in, tot_na_in,    & !intent-outs
         tot_ca_in, tot_lim2_in, tot_lim1_in, tot_aro1_in, tot_api1_in, tot_api2_in )  !zlu soa
    ...
    do ibin = 1, nbin_a
       ...
       if (irepeat_mosaic == 1) then
          ...
          if ( (jaerosolstate(ibin) .eq. no_aerosol) .or.   &
               (min(mass_dry_a(ibin),vol_dry_a(ibin)) .le. 1.0e-35_r8) ) then
    ...
    do ibin = 1, nbin_a
       if(jaerosolstate(ibin).ne.no_aerosol) then 
          ...
          elseif (mhyst_method == mhyst_uporlo_waterhyst) then
             water_a_up(ibin)   = aerosol_water_up(ibin,electrolyte,aer,kappa_nonelectro,a_zsr)      ! at 60% RH
             if (water_a_hyst(ibin) <= 0.5_r8*water_a_up(ibin)) then
                jhyst_leg(ibin) = jhyst_lo
                water_a_hyst(ibin) = 0.0_r8
             else
                jhyst_leg(ibin) = jhyst_up
...
!   use module_print_aer,         only: print_aer
  ...
  subroutine initialize_mosaic_variables(                                          & !intent-ins
    ...
    do ibin = 1, nbin_a
       ...
       mass_dry_a(ibin)     = 0.0_r8
       mass_soluble_a(ibin) = 0.0_r8
       dens_dry_a(ibin)     =-1.0_r8

       do je = 1, nelectrolyte
          electrolyte(je,jtotal,ibin)  = 0.0_r8
          electrolyte(je,jsolid,ibin)  = 0.0_r8
          electrolyte(je,jliquid,ibin) = 0.0_r8
          activity(je,ibin)            = 0.0_r8
          gam(je,ibin)                 = 0.0_r8
       enddo

       gam_ratio(ibin)   = 0.0_r8

       do iv = 1, ngas_ioa
          flux_s(iv,ibin)   = 0.0_r8
          flux_l(iv,ibin)   = 0.0_r8
          kg(iv,ibin)       = 0.0_r8
          phi_volatile_s(iv,ibin) = 0.0_r8
          phi_volatile_l(iv,ibin) = 0.0_r8
          volatile_s(iv,ibin) = 0.0_r8
       enddo
 ...
 ! zlu soa
    real(r8), intent(out) :: tot_lim1_in, tot_aro1_in, tot_api1_in, tot_api2_in   
 ...
 ! zlu soa
    tot_lim1_in= gas(ilim1_g)
    tot_aro1_in= gas(iaro1_g)
    tot_api1_in= gas(iapi1_g)
    tot_api2_in= gas(iapi2_g)
 ...
 !zlu soa
       tot_lim1_in= tot_lim1_in+ aer(ilim1_a,jtotal,ibin)
       tot_aro1_in= tot_aro1_in+ aer(iaro1_a,jtotal,ibin)
       tot_api1_in= tot_api1_in+ aer(iapi1_a,jtotal,ibin)
       tot_api2_in= tot_api2_in+ aer(iapi2_a,jtotal,ibin)
  ...
  subroutine overall_massbal_out( iprint_input, mbin, isteps_ASTEM, aer, gas, &
    tot_so4_in, tot_no3_in, tot_cl_in, tot_nh4_in, tot_na_in, tot_ca_in, tot_lim2_in, &
     tot_lim1_in, tot_aro1_in, tot_api1_in, tot_api2_in, mosaic_vars_aa )   ! zlu soa
    !      include 'v33com'
    !      include 'v33com3'
    !      include 'v33com9a'
    !      include 'v33com9b'
    ...
    real(r8), intent(inout) ::  tot_lim1_in, tot_aro1_in, tot_api1_in, tot_api2_in ! zlu soa
    type (mosaic_vars_aa_type), intent(inout) :: mosaic_vars_aa
    ...
    !zlu soa
    real(r8) :: tot_lim1_out, tot_aro1_out, tot_api1_out, tot_api2_out
    real(r8) :: reldiff_lim1, reldiff_aro1, reldiff_api1, reldiff_api2
    real(r8) ::  diff_lim1, diff_aro1, diff_api1, diff_api2   
    ...
    tot_lim1_out= gas(ilim1_g)
    tot_aro1_out= gas(iaro1_g)
    tot_api1_out= gas(iapi1_g)
    tot_api2_out= gas(iapi2_g)
    tot_na_out  = 0.0_r8
    tot_ca_out  = 0.0_r8
    ...
    do ibin = 1, nbin_a
       ...
       tot_lim1_out= tot_lim1_out+ aer(ilim1_a,jtotal,ibin)
       tot_aro1_out= tot_aro1_out+ aer(iaro1_a,jtotal,ibin)
       tot_api1_out= tot_api1_out+ aer(iapi1_a,jtotal,ibin)
       tot_api2_out= tot_api2_out+ aer(iapi2_a,jtotal,ibin)
    enddo
    ...
    diff_lim1= tot_lim1_out- tot_lim1_in
    diff_aro1= tot_aro1_out- tot_aro1_in
    diff_api1= tot_api1_out- tot_api1_in
    diff_api2= tot_api2_out- tot_api2_in

    reldiff_so4 = 0.0_r8
    tmpa = max( tot_so4_in, tot_so4_out )
    if (tmpa .gt. 1.0e-25_r8) reldiff_so4 = diff_so4/tmpa

    reldiff_no3 = 0.0_r8
    tmpa = max( tot_no3_in, tot_no3_out )
    if (tmpa .gt. 1.0e-25_r8) reldiff_no3 = diff_no3/tmpa

    reldiff_cl = 0.0_r8
    tmpa = max( tot_cl_in, tot_cl_out )
    if (tmpa .gt. 1.0e-25_r8) reldiff_cl = diff_cl/tmpa

    reldiff_nh4 = 0.0_r8
    tmpa = max( tot_nh4_in, tot_nh4_out )
    if (tmpa .gt. 1.0e-25_r8) reldiff_nh4 = diff_nh4/tmpa

    reldiff_lim2 = 0.0_r8
    tmpa = max( tot_lim2_in, tot_lim2_out )
    if (tmpa .gt. 1.0e-25_r8) reldiff_lim2 = diff_lim2/tmpa

    reldiff_lim1 = 0.0_r8
    tmpa = max( tot_lim1_in, tot_lim1_out )
    if (tmpa .gt. 1.0e-25_r8) reldiff_lim1 = diff_lim1/tmpa

    reldiff_aro1 = 0.0_r8
    tmpa = max( tot_aro1_in, tot_aro1_out )
    if (tmpa .gt. 1.0e-25_r8) reldiff_aro1 = diff_aro1/tmpa

    reldiff_api1 = 0.0_r8
    tmpa = max( tot_api1_in, tot_api1_out )
    if (tmpa .gt. 1.0e-25_r8) reldiff_api1 = diff_api1/tmpa

    reldiff_api2 = 0.0_r8
    tmpa = max( tot_api2_in, tot_api2_out )
    if (tmpa .gt. 1.0e-25_r8) reldiff_api2 = diff_api2/tmpa

    reldiff_na = 0.0_r8
    tmpa = max( tot_na_in, tot_na_out )
    if (tmpa .gt. 1.0e-25_r8) reldiff_na = diff_na/tmpa

    reldiff_ca = 0.0_r8
    tmpa = max( tot_ca_in, tot_ca_out )
    if (tmpa .gt. 1.0e-25_r8) reldiff_ca = diff_ca/tmpa

    reldiff_max = max( abs(reldiff_so4), abs(reldiff_no3), abs(reldiff_cl), &
                       abs(reldiff_nh4), abs(reldiff_na),  abs(reldiff_ca), &
                       abs(reldiff_lim2) ,&
                        abs(reldiff_lim1), abs(reldiff_aro1), abs(reldiff_api1), abs(reldiff_api2)) !zlu soa
    ...
    diff_max = max( abs(diff_so4)*mw_aer_mac(iso4_a), abs(diff_no3)*mw_aer_mac(ino3_a), abs(diff_cl)*mw_aer_mac(icl_a), &
                    ...
                    abs(diff_lim2)*mw_aer_mac(ilim2_a) ,&
                    abs(diff_lim1)*mw_aer_mac(ilim1_a) , abs(diff_aro1)*mw_aer_mac(iaro1_a) , &
                    abs(diff_api1)*mw_aer_mac(iapi1_a) , abs(diff_api2)*mw_aer_mac(iapi2_a)  )*1.0e-3_r8   ! diff_so4, etc. = nmol/m^3 ; diff_max = ug/m^3
...
!   if( reldiff_max .gt. 1.0e-4 ) then
    if( reldiff_max .gt. overall_massbal_rtoler .and. &
       ...
       if (mosaic_vars_aa%it_mosaic <= 48) then
          if(iprint_input .eq. mYES)then
             ...
             ! zlu soa
             write(lunerr,'(a,1p,3e11.3)') 'reldiff_lim1= ', reldiff_lim1, tot_lim1_in, tot_lim1_out
             write(lunerr,'(a,1p,3e11.3)') 'reldiff_aro1= ', reldiff_aro1, tot_aro1_in, tot_aro1_out
             write(lunerr,'(a,1p,3e11.3)') 'reldiff_api1= ', reldiff_api1, tot_api1_in, tot_api1_out
             write(lunerr,'(a,1p,3e11.3)') 'reldiff_api2= ', reldiff_api2, tot_api2_in, tot_api2_out
             !          call print_input
             iprint_input = mNO
          endif
       endif
  ...
  subroutine check_aerosol_mass(ibin, jaerosolstate,jphase,aer,num_a, mass_dry_a )
    ...
    aer_H = (2._r8*aer(iso4_a,jtotal,ibin) +   &
         aer(ino3_a,jtotal,ibin) +   &
         aer(icl_a,jtotal,ibin)  +   &
         aer(imsa_a,jtotal,ibin) +   &
         2._r8*aer(ico3_a,jtotal,ibin))-   &
         (2._r8*aer(ica_a,jtotal,ibin)  +   &
    ...
    mass_dry_a(ibin) = mass_dry_a(ibin)*1.e-15_r8      ! g/cc(air)

    if(drymass .lt. mass_cutoff)then                        ! bin mass is too small
       jaerosolstate(ibin) = no_aerosol
       jphase(ibin) = 0
       if(drymass .eq. 0._r8)num_a(ibin) = 0.0_r8
    endif
  ...
  subroutine conform_aerosol_number(ibin,jaerosolstate,aer,num_a,vol_dry_a, Dp_dry_a)
    ...
    if (msize_framework /= msectional) then
       if (num_a(ibin) > 0.0_r8) return
    end if

    vol_dry_a(ibin)  = 0.0_r8          ! initialize to 0.0
    ...
    vol_dry_a(ibin) = vol_dry_a(ibin)*1.e-15_r8                           ! cc(aer)/cc(air)


    if (msize_framework /= msectional) then
       ! unstructured or modal - set (initialize) number only when incoming value is zero
       if (num_a(ibin) <= 0.0_r8) then
  ...
  subroutine calc_aerosol_dry_density(ibin,aer,dens_dry_a)
    ...
    ! calculate dry volume concentration
    aer_H = ( 2._r8*max( 0.0_r8, aer(iso4_a,jtotal,ibin) ) +   &
                 max( 0.0_r8, aer(ino3_a,jtotal,ibin) ) +   &
                 max( 0.0_r8, aer(icl_a,jtotal,ibin) )  +   &
                 max( 0.0_r8, aer(imsa_a,jtotal,ibin) ) +   &
              2._r8*max( 0.0_r8, aer(ico3_a,jtotal,ibin) ) )   &
          - ( 2._r8*max( 0.0_r8, aer(ica_a,jtotal,ibin) )  +   &
    ...
    if (min(tmp_mass,tmp_volu) >= 1.0e-20_r8) then
  ...
  subroutine degas_acids(jp,ibin,XT,aer,gas,electrolyte)
    ...
    electrolyte(jhno3,jp,ibin) = 0.0_r8
    electrolyte(jhcl,jp,ibin)  = 0.0_r8
...
! bound temperature between 220 and 330 K  ! 6/3/2015 RAZ
    ...
    tr = 298.15_r8                       ! reference temperature
    rt = 82.056_r8*T_K/(1.e9_r8*1.e6_r8)   ! [m^3 atm/nmol]

    ! gas-liquid
    Keq_gl(1)= 1.0_r8                                        ! Kelvin Effect (default)
    Keq_gl(2)= fn_Keq(57.64_r8, 13.79_r8, -5.39_r8,T_K)*rt     ! NH3(g)  <=> NH3(l)
    Keq_gl(3)= fn_Keq(2.63e6_r8,  29.17_r8, 16.83_r8,T_K)*rt     ! HNO3(g) <=> NO3- + H+
    Keq_gl(4)= fn_Keq(2.00e6_r8,  30.20_r8, 19.91_r8,T_K)*rt     ! HCl(g)  <=> Cl- + H+

    ! liquid-liquid
    Keq_ll(1)= fn_Keq(1.0502e-2_r8, 8.85_r8, 25.14_r8,T_K)      ! HSO4- <=> SO4= + H+
    Keq_ll(2)= fn_Keq(1.805e-5_r8, -1.50_r8, 26.92_r8,T_K)      ! NH3(l) + H2O = NH4+ + OH-
    Keq_ll(3)= fn_Keq(1.01e-14_r8,-22.52_r8, 26.92_r8,T_K)      ! H2O(l) <=> H+ + OH-
    ...
    ! solid-gas
    Keq_sg(1)= fn_Keq(4.72e-17_r8,-74.38_r8,6.12_r8,T_K)/rt**2  ! NH4NO3<=>NH3(g)+HNO3(g)
    Keq_sg(2)= fn_Keq(8.43e-17_r8,-71.00_r8,2.40_r8,T_K)/rt**2  ! NH4Cl <=>NH3(g)+HCl(g)


    ! solid-liquid
    Keq_sl(jnh4so4) = fn_Keq(1.040_r8,-2.65_r8, 38.57_r8, T_K)  ! amSO4(s) = 2NH4+ + SO4=
    Keq_sl(jlvcite) = fn_Keq(11.8_r8, -5.19_r8, 54.40_r8, T_K)  ! lvcite(s)= 3NH4+ + HSO4- + SO4=
    Keq_sl(jnh4hso4)= fn_Keq(117.0_r8,-2.87_r8, 15.83_r8, T_K)  ! amHSO4(s)= NH4+ + HSO4-
    Keq_sl(jnh4msa) = 1.e15_r8                                      ! NH4MSA(s)= NH4+ + MSA-
    Keq_sl(jnh4no3) = fn_Keq(12.21_r8,-10.4_r8, 17.56_r8, T_K)  ! NH4NO3(s)= NH4+ + NO3-
    Keq_sl(jnh4cl)  = fn_Keq(17.37_r8,-6.03_r8, 16.92_r8, T_K)  ! NH4Cl(s) = NH4+ + Cl-
    Keq_sl(jna2so4) = fn_Keq(0.491_r8, 0.98_r8, 39.75_r8, T_K)  ! Na2SO4(s)= 2Na+ + SO4=
    Keq_sl(jnahso4) = fn_Keq(313.0_r8, 0.8_r8,  14.79_r8, T_K)  ! NaHSO4(s)= Na+ + HSO4-
    Keq_sl(jna3hso4)= 1.e15_r8                                      ! Na3H(SO4)2(s) = 2Na+ + HSO4- + SO4=
    Keq_sl(jnamsa)  = 1.e15_r8                                      ! NaMSA(s) = Na+ + MSA-
    Keq_sl(jnano3)  = fn_Keq(11.95_r8,-8.22_r8, 16.01_r8, T_K)  ! NaNO3(s) = Na+ + NO3-
    Keq_sl(jnacl)   = fn_Keq(38.28_r8,-1.52_r8, 16.89_r8, T_K)  ! NaCl(s)  = Na+ + Cl-
    Keq_sl(jcacl2)  = fn_Keq(8.0e11_r8,  32.84_r8,44.79_r8, T_K)  ! CaCl2(s) = Ca++ + 2Cl-
    Keq_sl(jcano3)  = fn_Keq(4.31e5_r8,   7.83_r8,42.01_r8, T_K)  ! Ca(NO3)2(s) = Ca++ + 2NO3-
    Keq_sl(jcamsa2) = 1.e15_r8                                ! CaMSA2(s)= Ca+ + 2MSA-

    ! vapor pressures of soa species
    Po_soa(iaro1_g) = fn_Po(5.7e-5_r8, 156.0_r8, T_K) ! [Pascal]
    Po_soa(iaro2_g) = fn_Po(1.6e-3_r8, 156.0_r8, T_K) ! [Pascal]
    Po_soa(ialk1_g) = fn_Po(5.0e-6_r8, 156.0_r8, T_K) ! [Pascal]
    Po_soa(iole1_g) = fn_Po(5.0e-6_r8, 156.0_r8, T_K) ! [Pascal]
    Po_soa(iapi1_g) = fn_Po(4.0e-6_r8, 156.0_r8, T_K) ! [Pascal]
    Po_soa(iapi2_g) = fn_Po(1.7e-4_r8, 156.0_r8, T_K) ! [Pascal]
    Po_soa(ilim1_g) = fn_Po(2.5e-5_r8, 156.0_r8, T_K) ! [Pascal]
    Po_soa(ilim2_g) = fn_Po(1.2e-4_r8, 156.0_r8, T_K) ! [Pascal]

    sat_factor = 0.5_r8  ! = 1.0 for original SORGAM parameters
    do iv = isoa_first, ngas_volatile
       !       sat_soa(iv) = 1.e9*Po_soa(iv)/(8.314*T_K)  ! [nmol/m^3(air)]
       sat_soa(iv) = sat_factor * 1.e9_r8*Po_soa(iv)/(8.314_r8*T_K)  ! [nmol/m^3(air)]
    enddo

    if ( use_cam5mam_soa_params > 0 ) then
!       Po_soa(ilim2_g) = fn_Po(1.0e-10_r8, 156.0_r8, T_K) ! [Pascal]
!       sat_soa(ilim2_g) = 1.e9_r8*Po_soa(ilim2_g)/(8.314_r8*T_K)  ! [nmol/m^3(air)]

       Po_soa(ilim2_g) = fn_Po(9.9127e-8_r8, 153.0_r8, T_K) ! [Pascal]
       sat_soa(ilim2_g) = 1.e9_r8*Po_soa(ilim2_g)/(8.314_r8*T_K)

!zlu soa   
       Po_soa(ilim1_g) = fn_Po(9.9127e-7_r8, 142.0_r8, T_K) ! [Pascal]
       sat_soa(ilim1_g) = 1.e9_r8*Po_soa(ilim1_g)/(8.314_r8*T_K)

       Po_soa(iaro1_g) = fn_Po(9.9127e-6_r8, 131.0_r8, T_K) ! [Pascal]
       sat_soa(iaro1_g) = 1.e9_r8*Po_soa(iaro1_g)/(8.314_r8*T_K)

       Po_soa(iapi1_g) = fn_Po(9.9127e-5_r8, 120.0_r8, T_K) ! [Pascal]
       sat_soa(iapi1_g) = 1.e9_r8*Po_soa(iapi1_g)/(8.314_r8*T_K)

       Po_soa(iapi2_g) = fn_Po(9.9127e-4_r8, 109.0_r8, T_K) ! [Pascal]
       sat_soa(iapi2_g) = 1.e9_r8*Po_soa(iapi2_g)/(8.314_r8*T_K)
    ...
    term = (647.15_r8 - T_K)/647.15_r8
    sigma_water = 0.2358_r8*term**1.256_r8 * (1._r8 - 0.625_r8*term) ! surface tension of pure water in N/m
  ...
  function fn_Keq(Keq_298, a, b, T)
    ...
    tt = 298.15_r8/T
    fn_Keq = Keq_298*exp(a*(tt-1._r8)+b*(1._r8+log(tt)-tt))
  ...
  function fn_Po(Po_298, DH, T)   ! TOUCH
    ...
    fn_Po = Po_298*exp(-(DH/8.314e-3_r8)*(1._r8/T - 3.354016435e-3_r8))
  ...
  function drh_mutual(j_index,T_K)            ! TOUCH
    ...
    if(j_index .eq. 7 .or. j_index .eq. 8 .or.   &
       ...
       drh_mutual = 10.0_r8  ! cano3 or cacl2 containing mixtures
...
! function bin_molality
  ...
  subroutine quadratix(a,b,c, qx1,qx2)
    ...
    if(abs(x) .lt. 1.e-6_r8)then
       ...
       qx1 = ((-b)+sqrt((b*b)-(4._r8*a*c)))/   &
            (2._r8*a)
       qx2 = ((-b)-sqrt((b*b)-(4._r8*a*c)))/   &
            (2._r8*a)
  ...
  subroutine aerosol_optical_properties(                                 &
    ...
    complex(r8),  intent(in), dimension(naercomp)   :: ref_index_a
    complex(r8),  intent(inout), dimension(nbin_a_max) :: ri_shell_a, ri_avg_a, ri_core_a
